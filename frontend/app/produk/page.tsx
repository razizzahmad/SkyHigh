...
export default function ProdukPage() {
  // Ambil data produk
  const { data: produkData, error: produkError, mutate: mutateProduk } = useSWR(
  ...
  // Gabungkan data produk dengan data petani berdasarkan petaniId
  const produkDenganPetani = produkData.data_user.map((produk: any) => {
    const petani = petaniData.data_user.find((p: any) => p.id === produk.petaniId);
    return {
      ...produk,
    ...
    <div className="min-h-screen bg-gradient-to-br from-green-100 via-green-50 to-white p-6 flex flex-col items-center justify-center">
      <div className="w-full max-w-6xl bg-white p-6 rounded-2xl shadow-lg border border-lime-200">
        ...
        <div className="flex justify-end mb-4">
          <button className="bg-lime-600 hover:bg-lime-700 text-white font-medium px-4 py-2 rounded-xl shadow transition">
            Tambah Produk
          </button>
        </div>
